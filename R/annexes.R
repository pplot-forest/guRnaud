...
#' @description La fonction permet de débarasser une chaîne de caractères (comme un nom de massif forestier, de GF, etc.) des caractères pouvant gêner le traitement des données (espaces, caractères spéciaux, ...).
...
#' Fonction pour filtrer les données par forêt
#' @description La fonction filtre un ensemble de tables pour ne garder que les données relatives à un/des numéro(s) de forêt donné(s).
#' @return La fonction renvoie toutes les tables filtrées avec la liste des forêts de l'argument 'forest_list'
#' @param tables = liste des tables à filtrer
#' @param forest_list = liste des forêts à utiliser dans le filtre
#' @param cycle = numéro du passage en inventaire
#' @import dplyr
#' @import rlang
#' @export
